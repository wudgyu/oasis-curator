...
- 🤖 **文档处理 Agent**：上传混合文档（中英/含表格），LLM 自主编排解析/翻译/摘要/入库
  （原生 Function Calling 优先，模型不支持时自动降级为 Prompt 注入；前端实时展示每步工具调用）
...
> ✅ **RAG 文档问答（v0.4）已完成**：文档上传 → 解析（PDF/Word/TXT/Markdown，表格转 Markdown
> 参与检索）→ 切分 → Embedding（智谱 embedding-2 / Ollama bge-m3 / MiniLM 自动选择）→
> ChromaDB 入库 → 检索 → LLM 重排序 → 流式生成带 `[来源: 文件名, 第N段]` 引用，
> 文档外问题明确拒答。前端提供文档管理页（拖拽上传 + 可见性配置）与问答页
> （会话列表 + 逐字流式 + 引用点击查看原文）。
>
> 文档级权限：可见性三态（租户内公开 / 仅上传者可见 / 指定角色可见），检索与列表按同一套规则过滤。
> 评估脚本实测：召回率 100%、答案质量 100%、引用正确率 100%、拒答正确；
> 权限自测 26 项断言、流式与会话 28 项断言、多租户隔离 10 项断言全部通过。
...
- [x] **v0.4** · RAG 文档问答（含文档级权限、会话历史、流式问答与前端集成）✅ 已完成
- [x] **v0.5** · 文档处理 Agent（Function Calling 自主编排）✅ 已完成
